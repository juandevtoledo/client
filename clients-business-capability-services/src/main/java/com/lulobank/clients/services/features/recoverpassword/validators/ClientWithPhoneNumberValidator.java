...
public class ClientWithPhoneNumberValidator implements Validator<RecoverPassword> {
  @Override
  public ValidationResult validate(RecoverPassword recoverPassword) {
    try {
      UtilValidators.validatePhoneNumber(recoverPassword.getPhoneNumber());
    } catch (Exception e) {
      return new ValidationResult(
          ClientErrorResultsEnum.CLIENT_WRONG_PHONENUMBER_FORMAT.name(),
